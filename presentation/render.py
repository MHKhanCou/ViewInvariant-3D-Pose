...
def fig_interface(out="fig_interface.png", dpi=200):
    """
    The demonstration interface itself, from screen captures.

    Figure fig_app shows what the application returns; this shows the thing a
    user operates. Three captures are cropped to their informative regions and
    stacked. The captures are the only images in this report not generated by
    code, because a running Gradio window cannot be rendered from a script.
    """
    from PIL import Image as PILImage

    shots = sorted(glob.glob(os.path.join(REPO_ROOT, "demo", "Screenshot*.png")))
    if len(shots) < 3:
        raise RuntimeError("expected three screen captures in demo/, found %d"
                           % len(shots))
    a, b, c = (np.array(PILImage.open(s).convert("RGB")) for s in shots)

    # Crop each to what it contributes. Fractions rather than pixels so a
    # recapture at a different resolution still lands in roughly the right place.
    def frac(img, x0, x1, y0, y1):
        h, w = img.shape[:2]
        return img[int(h * y0):int(h * y1), int(w * x0):int(w * x1)]

    top = frac(a, 0.0, 0.99, 0.0, 1.0)            # header, tabs, input, toggle
    ctrl = frac(c, 0.0, 0.47, 0.0, 0.25)          # the display-only controls
    # Cropped above the caveat line: that line named a section number which the
    # report has since renumbered, and the application no longer prints one.
    ...
    ax = fig.add_subplot(gs[0, :])
    ax.imshow(top)
    ax.set_title("The interface: input, the Representation control, and the "
                 "interactive 3D viewer", fontsize=10.5, pad=7)
    ax2 = fig.add_subplot(gs[1, 0])
    ax2.imshow(ctrl)
    ax2.set_title("Display-only controls", fontsize=10.5, pad=7)
    ax3 = fig.add_subplot(gs[1, 1])
    ax3.imshow(read)
    ax3.set_title("The plausibility readout and its six components",
                  fontsize=10.5, pad=7)
