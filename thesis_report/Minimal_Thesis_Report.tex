...
%  addendum: the failure-support map and the routing rule (pre-registered
%  7 Aug 2026).
% =====================================================================
...
\item \textbf{[Systems]} \textbf{An evidence trail built so the claims can be checked rather than believed.} Zero learned parameters added, 402 floating-point operations per frame, 258 numerical claims checked against stored artifacts by an automated audit, 76 tests, and nine pre-registrations committed to version history before the experiments they governed, several of which failed.
\end{itemize}
...
Every experiment in this report was pre-registered: its criterion and all plausible readings were written down and committed to version control \emph{before} the experiment ran. Nine such pre-registrations govern Chapter 5, several of which failed their own criteria; four more were run after the freeze (Sections 5.8), two of which are the subject of this report's addendum. Every reported number is recomputed from stored result files by an automated audit of 258 claims, alongside 76 unit tests; both pass. No number in the report was typed by hand.
