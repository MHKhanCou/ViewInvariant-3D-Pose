...
The baseline reproduces the published MotionAGFormer-XS figures to within 0.05~mm: MPJPE 45.149~mm against the reported 45.1, and P-MPJPE 36.892~mm against 36.9. The published figures carry one decimal, so a closer comparison is available only against the backbone's own evaluation script, which our pipeline matches to three decimals. This is what makes the negative results interpretable: a negative result is only informative if the apparatus is not what failed.
