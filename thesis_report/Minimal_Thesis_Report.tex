...
\caption{The pipeline end to end. RGB video enters a frozen 2D detector; the keypoints are converted to the Human3.6M layout and lifted by a frozen MotionAGFormer. The only step this thesis adds is the last one, where the predicted skeleton is re-expressed in a frame built from the subject's own hips and torso. Nothing here is trained, no camera parameters are used at any stage, and the estimator is never modified.}
\label{fig:pipeline}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Every claim tested in this project and its verdict. Pre-registered experiments are marked \textsc{pr}.}
\label{tab:summary}
...
\caption{The headline result as a distribution rather than a mean. Each point is one of the 180 held-out camera pairs, before and after canonicalization. The single pair that does not improve is visible, and is reported rather than trimmed.}
\label{fig:distances}
\end{figure}
...
\caption{What the method does, on photographs rather than stick figures. One instant of MPI-INF-3DHP seen by all eight synchronized cameras. Below left, the eight predictions in their own camera frames: the same pose eight times, and they do not overlap. Below right, the same eight after canonicalization, each camera processed independently, in ignorance of the others and without calibration. Mean pairwise distance falls from 0.149 to 0.095 in the normalised units the cache stores, a reduction of 36.6 percent. This is a single frame and it illustrates rather than measures; the measured figures over 180 held-out camera pairs are in Table \ref{tab:baseproposed}.}
\label{fig:realview}
\end{figure}
...
\caption{Where the method sits against its floor. The per-frame Procrustes oracle is allowed to see both views and is therefore unreachable by any single-view method; it bounds how much of the remaining disagreement is removable by rotation at all.}
\label{fig:oracle}
\end{figure}
...
\begin{table}[H]
\centering
\caption{The central result under both backbones. Same code, same protocol, same 180 held-out camera pairs.}
\label{tab:backbone}
\begin{tabular}{lcc}
\toprule
...
\caption{The retraction. A temporal bone-length signal that looked like a genuine error predictor on the first dataset does not survive on the second, and every pre-registered criterion fails there.}
\label{fig:bone}
\end{figure}
...
\centering
\small
\caption{Every frame construction against Kabsch-to-template, scored on the nine joints no construction builds from. Gain is positive when the frame is closer than the template.}
\label{tab:bestframe}
\begin{tabular}{lcccc}
\toprule
...
\centering
\small
\caption{Left-side-only distal corruption. The anatomical frame is flat by construction, since it reads none of the corrupted joints.}
...
\centering
\small
\caption{Laterality at matched joint count. Positive would mean one-sided corruption damages the template alignment more, which is what was predicted.}
...
\textbf{Laterality moves the template alignment by at most 1.53~mm. The gap
between the two methods is 41.49~mm.} The lever is roughly twenty-seven times too
small, so no manipulation of corruption geometry in this family could have closed
it. That is a quantitative reason to stop rather than another qualitative failure,
and it is worth more than the searches that preceded it: had the effect size been
measured first, several of those experiments would not have been run.
...
Six pre-registered searches have now looked for a pose regime in which the anatomical construction is preferable to the template baseline, and none found one at a severity committed to in advance on both backbones. The last of them measured why: the largest effect available from manipulating corruption geometry is 1.5~mm, against a 41.5~mm deficit. That is the difference between a search abandoned and a search closed, and it is the more useful of the two outcomes.
