...
On Human3.6M, which played no part in developing the method, canonicalization reduces cross-view distance by 72.2 percent over 180 held-out camera pairs, improving 179 of them. Estimation accuracy is unchanged, because the estimator is frozen: what improves is agreement between cameras, not correctness. A single-view baseline meeting the same requirements (Kabsch alignment to one skeleton) beats our frame on all 180 pairs under both backbones. Six pre-registered searches for a regime where ours is preferable have failed. A reliability score is falsified as an accuracy predictor across five independent tests, and a bone-length signal that correlated highly on one dataset fails to replicate on another and is retracted.

The contribution is an experimental boundary rather than a method: axis length governs the choice between frame constructions and nothing finer, articulation rather than radius governs which joint disagrees, and the two alignments fail on disjoint joint supports. Because those failure supports are disjoint, a fixed rule selecting between the two attains the better one at 19 of 20 evaluated cells; this is reported as exploratory, because the per-joint confidence the rule consumes is simulated rather than measured. Seventeen experiments were pre-registered with their criteria committed before each run, more than half failed, and every measured number is re-derived from stored artifacts by an automated audit.
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{images/fig_architecture.png}
\caption[Where the contribution sits in the pipeline]{Where the contribution
sits in the pipeline. Everything above the dashed line is trained by its
authors and used here frozen and unmodified. Everything below it is this work,
and it adds no trained parameters, consumes no labels and uses no camera
parameters. The identity beneath the diagram is why: the frame is built from
the joints, so it rotates with them, and the unknown rotation between two
cameras cancels rather than being estimated.}
\label{fig:architecture}
\end{figure}

\section{Contributions}

The geometry this project builds on is not ours: the frame is TRIAD \cite{triad}, the primary-axis rule is Shuster and Oh's \cite{shuster}, and the propagation of landmark error into frame orientation is biomechanics \cite{dellacroce1999,dellacroce2005}. What we contribute is the transfer of that reasoning to joints inferred by a frozen network, and principally the experimental boundary of how far it carries.

\begin{itemize}
\item \textbf{[Analysis]} Delimitation of the axis-length principle: axis length governs the choice between constructions (not which joint disagrees, which is governed by articulation).
\item \textbf{[Validation]} Cross-view replication on Human3.6M showing canonicalization improves cross-view distance on 179 of 180 held-out pairs, with no per-frame training or calibration.
\item \textbf{[Empirical finding]} A reliability score falsified as an accuracy predictor but validated as a gate for canonicalization confidence.
\item \textbf{[Negative result]} Retraction of a temporal bone-length signal that failed to transfer across datasets.
\item \textbf{[Engineering]} Calibration-free multi-view fusion showing that statistic choice (mean vs.\ median) determines whether combining views helps.
\item \textbf{[Systems]} A reproducible evaluation harness over two datasets and two frozen backbones, with cross-dataset replication, and an evidence trail behind it: 304 audited claims re-derived from stored artifacts, 76 passing tests, and 17 pre-registered experiments with transparent failure reporting.
\end{itemize}

\noindent\textbf{Position of this work.} Existing literature addresses anatomical canonicalization or geometric alignment or reliability estimation individually. This thesis contributes the first systematic study of all three---anatomical canonicalization, calibration-free multi-view fusion, and per-frame confidence estimation---as a unified post-processing framework evaluated across two datasets, two backbone architectures, and pre-registered boundary tests. No prior work operates under the constraint of zero training, zero labels, and zero camera parameters simultaneously on a frozen estimator.
...
\caption[The pipeline on uncontrolled footage]{The pipeline on uncontrolled footage, outside either evaluation dataset. Left: the input frame with the detected 2D keypoints. Middle: the lifted 3D prediction in the camera's frame. Right: the same prediction after canonicalization. The datasets below are laboratory multi-camera rigs; this is what the same code does on an ordinary video.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{images/fig_app.png}
