...
|---|---|
| "A Lightweight, Training-Free, Reliability-Aware Geometric Canonicalization Framework…" | the thesis's own title |
...
1. The text describes what was actually done, by whom, including six failed
   pre-registered experiments and a retraction. Editing it to alter an
...
- `evaluation/audit_numbers.py` re-derives all 248 numerical claims from stored
...
- Six of nine pre-registered experiments failed and are reported as failures,
  including a competing baseline that beats the proposed method on every one of
  180 pairs and all 15 actions.
...
> (`evaluation/audit_numbers.py`, 248 claims) and is reproducible from the
