...
\begin{itemize}
\item \textbf{A bridge from attitude determination to anatomical frames.} The construction used here is TRIAD [19], and the rule that the better-known direction should be primary is due to Shuster and Oh [20]. Neither is claimed. What that literature cannot supply is the accuracy of an anatomical direction, because it assumes each sensor reports its own uncertainty. We derive it: a direction read from two joints separated by $L$, each carrying independent noise $\sigma$, is uncertain by approximately $2\sigma/L$, which identifies segment length as the design variable for a skeleton. On two independently trained backbones of different architecture and nineteen-fold different size, rank correlation with cross-view consistency is 0.904 and 0.880. The ordering is what we claim; the fitted constant is not determined tightly enough by eight frame definitions to be called physical, and Section \ref{sec:multilandmark} withdraws an earlier argument that rested on it. Acting on the ordering raises the multi-scale improvement from 25.6 to 55.1 percent at no parameter cost.
...
Third, and this is where the transfer stops. The attitude literature takes the angular uncertainty of each direction measurement as given, since a star tracker or magnetometer reports it. We cannot, because our directions are computed from the positions of two estimated joints. Section \ref{sec:axistheory} therefore derives the missing step, propagating per-joint position noise into the angular uncertainty of the segment direction as $2\sigma/L$, which is what makes anatomical segment length the operative design variable. That propagation is elementary in itself; what it buys is the bridge that lets sixty years of attitude-determination reasoning apply to a skeleton, and it is the bridge rather than either end that this report contributes.

We therefore claim neither the construction nor the principle that a frame should be built from its most accurate axis. Both are established. What we claim is the quantification of that principle for anatomical frames, where accuracy is governed by segment length, and the empirical boundary of where the reasoning holds, which Section \ref{sec:radial} shows is narrower than the attitude literature would suggest because a human body articulates and a spacecraft does not.
...
We are careful about what part of this is ours. The construction is TRIAD [19] and the rule that the better-determined direction should be primary is Shuster and Oh's [20], both predating this work by decades, and Section \ref{sec:triad} says so at length. Our contribution is the step between them and a skeleton: attitude determination assumes each direction arrives with a known uncertainty, which is true of a star tracker and false of a joint detector, and equation \eqref{eq:theta} supplies the missing propagation from joint noise to angular uncertainty. That is a small piece of mathematics. Its value is that it makes an established body of reasoning applicable to a problem it was not written for, and that the applicability turns out to be limited in a way the original literature could not have anticipated. It raises the multi-scale improvement from 25.6 to 55.1 percent at no parameter cost, and its ordering reproduces on two independently trained networks of different architecture and nineteen-fold different size, at rank correlations of $+0.904$ and $+0.880$.
