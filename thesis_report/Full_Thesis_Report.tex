...
\item A pre-registered abstention experiment which fails, and, in failing, establishes the narrower claim that the falsified reliability score does gate canonicalization quality, cutting mean cross-view distance by 22 percent at thirty percent abstention and surviving a confound control on both backbones.

\item A three-level delimitation of the axis-length principle, pre-registered at each level, establishing that it governs the choice of frame construction but neither which frame within a construction to trust nor which joint within a frame will disagree, the latter because joint-level disagreement is dominated by articulation rather than by frame geometry.
\end{itemize}
