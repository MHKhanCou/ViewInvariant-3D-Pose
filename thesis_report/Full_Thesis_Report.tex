...
\item \textbf{[Systems]} \textbf{A framework and an evidence trail built so the claims can be checked rather than believed.} Zero learned parameters added to a frozen estimator, 402 floating-point operations per frame, no labels and no camera parameters at any stage; 230 numerical claims checked against stored artifacts by an automated audit; 76 tests; and seven pre-registrations committed to version history before the experiments they govern, five of which failed their own criteria and one of which returned a competing method as the better one (Section \ref{sec:template}).
\end{itemize}
...
Two properties of the comparison belong with the number rather than after it. The template estimator fits its rotation by least squares over all seventeen joints, where ours reads two anatomical vectors, so it uses strictly more of each prediction. And it normalizes translation by the centroid of the pose while ours uses the root joint alone, so part of the gap may be better translation handling rather than better orientation; we have not separated the two. Both observations describe how the baseline wins, not whether it does.
...
\subsection{Two Ablations of That Baseline}
