...
Two caveats belong with it. The score contains axis conditioning among its six components, yet conditioning alone fails this test while the score passes, which means the discriminating work is being done by the other components rather than by the geometry the theory identifies. And triage is not calibration: nothing here supplies a coverage guarantee of the kind Section \ref{sec:relfails} notes conformal methods provide.
