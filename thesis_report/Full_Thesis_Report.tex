...
\item \textbf{[Systems]} \textbf{A framework and an evidence trail built so the claims can be checked rather than believed.} Zero learned parameters added to a frozen estimator, 402 floating-point operations per frame, no labels and no camera parameters at any stage; 167 numerical claims recomputed from stored artifacts by an automated audit; 72 tests; and four pre-registrations committed to version history before the experiments they govern, two of which the results then contradicted.
\end{itemize}
