...
Table \ref{tab:requirements} summarizes what each approach requires at deployment. No prior method operates on a frozen estimator with no training, no labels, and no camera parameters simultaneously. That requirement profile, rather than any individual mechanism, is the position this project occupies. 3DPCNet addresses the same problem and reports a hand-built anatomical baseline of the same family performing considerably worse than its learned module. Closer still, Wei et al. [25] transform predicted poses into consistent views before refining them, and do so first at the global body level and then at the level of local body parts, which is the same two-level decomposition this report arrives at in Sections \ref{sec:frame} and \ref{sec:h36mms}, from the same anatomical landmarks. We note it explicitly because a reader who found it independently would reasonably ask why it was absent. What differs is not the geometry but everything around it: their transformation feeds a trained correction network and a trained discriminator, so it is unavailable to a practitioner who cannot train, and the consistent views are an intermediate step rather than a quantity they evaluate. Our contribution is not the construction but the requirement profile that keeps it usable on a frozen estimator, and the experimental boundary of Chapter 5. The construction is therefore treated as an instrument rather than as a result. What it makes possible is the experimental question of Section \ref{sec:objectives}, since a frame with no trained parameters and no calibration allows every variable except the frame's own construction to be held fixed, and Chapter 5 uses that to establish where the geometry inherited in Section \ref{sec:triad} applies and where it stops.
...
\begin{table}[htbp]
\centering
\caption[Global frame from redundant landmarks]{Global frame from redundant landmarks. Paired gain is against the two-vector baseline on identical frames.}
...
\begin{table}[htbp]
\centering
\caption[Headroom above the per-segment Procrustes floor]{Headroom above the per-segment Procrustes floor. A ratio near one means the frame has already removed everything a rotation could, so the measurement cannot demonstrate anything further.}
