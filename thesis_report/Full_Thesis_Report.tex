...
\item \textbf{[Negative result]} \textbf{Articulation, not radius, governs per-joint disagreement.} The rigid-segment relation that displacement grows with distance from the frame origin [24] does not survive on a whole articulated skeleton. Joints rigid with the torso sit at a \emph{larger} mean radius than articulated joints and yet disagree across views by a factor of two and a half \emph{less}, on both backbones, with matched-radius pairs differing by one to three percent in radius and roughly a factor of two in distance. Neither the aerospace nor the single-segment biomechanical literature had reason to test this.
...
What remains ours is therefore narrower than a derivation, and we state it as three things. The first is the transfer: this reasoning was developed for markers placed on a body by a human examiner, whose error is palpation variability, and we apply it to joints inferred by a frozen network from a single image, whose error has a different origin and, as Section \ref{sec:relfails} shows, a different structure, being dominated by coherent depth ambiguity rather than by independent placement noise. The second is the design consequence for a canonical frame, connecting the biomechanical propagation result to TRIAD's primary-axis rule and to the choice of which anatomical axis to build from, which neither literature makes for this purpose. The third, and the one we regard as the actual contribution, is the empirical boundary of Section \ref{sec:radial}: the rigid-body pivot relation that biomechanics states for a segment does not survive on a whole articulated skeleton, where joints beyond a hinge disagree by a factor of two and a half more than joints at a larger radius that move rigidly with the torso. A spacecraft has no hinges, and a single bone segment has none either; a body does, and that is where the inherited reasoning stops.
