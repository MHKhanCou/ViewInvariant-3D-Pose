...
The primary contribution is a design principle for anatomical reference frames, derived and then bounded by experiment. The framework is the vehicle that makes it testable, and the negative results are reported at the same weight as the positive ones.

\begin{itemize}
\item \textbf{A transfer of established geometry to inferred anatomy, claimed as a transfer and not a discovery.} The construction is TRIAD [19]; the rule that the better-known direction should be primary is Shuster and Oh's [20]; the propagation of landmark error into anatomical frame orientation is developed in biomechanics [23, 24]. None of these is claimed here. What is ours is their application to joints inferred by a frozen network rather than placed by an examiner, where the error has a different origin, and the design consequence that follows for choosing a canonical axis. On two independently trained backbones of different architecture and nineteen-fold different size, rank correlation with cross-view consistency is 0.904 and 0.880. The ordering is what we claim; the fitted constant is not determined tightly enough by eight frame definitions to be called physical, and Section \ref{sec:multilandmark} withdraws an earlier argument that rested on it. Acting on the ordering raises the multi-scale improvement from 25.6 to 55.1 percent at no parameter cost.

\item \textbf{A three-level delimitation of that principle, pre-registered at each level.} It governs the choice between frame constructions. It does not govern which frame within a construction to trust, because there the axis is an anatomical near-constant, and it does not govern which joint within a frame will disagree, because joint-level disagreement is dominated by articulation: joints rigid with the torso sit at a larger radius than articulated ones yet disagree by a factor of 2.5 less. Two of the three tests are failures, and the boundary they establish is what tells a designer where to apply the principle.
...
\item A systematic falsification of the proposed quality signal along five independent axes with identification of its underlying cause, followed by a pre-registered experiment establishing the narrower claim that the same score does gate canonicalization quality, cutting mean cross-view distance by 22 percent at thirty percent abstention and surviving a confound control on both backbones.
...
One finding is genuinely a design result rather than an evaluation, and we regard it as the most transferable thing in this report, since any method that builds a frame from anatomical landmarks faces the same choice. The consistency of a body frame across viewpoints is governed by the length of the axis it is built from. We predicted it from the construction, discovered it through a bilateral asymmetry we had not gone looking for, confirmed it by correcting that asymmetry, generalized it to a second case the same reasoning identified, and then derived it from first principles as $\theta \approx 2\sigma/L$.
