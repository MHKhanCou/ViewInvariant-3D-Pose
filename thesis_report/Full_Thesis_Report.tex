...
\begin{itemize}
\item \textbf{[Analysis]} \textbf{A three-level delimitation of the axis-length principle, pre-registered at each level.} Axis length governs the choice \emph{between} frame constructions, at rank correlations of 0.904 and 0.880 on two independently trained backbones of different architecture and nineteen-fold different size. It does not govern which frame within one construction to trust, because there the axis is an anatomical near-constant. It does not govern which joint within a frame will disagree. Two of the three tests are failures, and the boundary they establish is the contribution: it tells a designer to apply the principle when choosing what to build the frame from, and not to expect it to do anything finer.
