...
\subsection*{A Longer Research Programme}

The results above isolate one problem more sharply than they solve it, and it is the natural subject of a larger study. Canonicalization succeeds, and every quality signal we constructed to accompany it fails for a single reason: geometric plausibility, whether measured within a frame or across time, is invariant to a depth error that is itself coherent. The one signal that does work, cross-view disagreement, needs a second camera and therefore does not apply to the monocular case that motivated the project. On the present evidence, a model that is confidently and smoothly wrong in depth cannot be detected from its own output. Learned uncertainty does not close the gap either, since it degrades under exactly the distribution shift it is introduced to catch [9].

Three directions follow. We state them as open questions rather than as claimed contributions, because establishing novelty would require a literature review broader than the one conducted here.

\noindent \textbf{Which constraints does a coherent depth error actually violate?} Bone-length inconsistency failed because a smoothly wrong depth estimate keeps bone lengths smooth, and Section \ref{sec:relfails} shows within-frame plausibility fails for the same reason. The productive question is therefore not which constraints a correct pose satisfies, but which ones a plausibly wrong pose cannot. Gravity is one candidate, since the vertical and the support surface are fixed in the world while a depth error tilts the reconstruction relative to both, so a violated ground-contact or verticality condition may carry information that internal consistency cannot. Dynamics is a second, since the accelerations implied by a wrongly scaled depth trajectory need not be physically attainable. Both are computable from a predicted skeleton without labels or training, so they preserve the deployment profile that distinguishes this framework, and both are falsifiable in the same way the bone-length signal was.

\noindent \textbf{Can a coverage guarantee be obtained without labels?} Conformal prediction supplies the guarantee this framework lacks, at the price of a labelled calibration set drawn from the deployment distribution, which is precisely what an unlabelled deployment does not have. Cross-view disagreement is label-free and ranks error at a Spearman coefficient of $+0.601$ on our data. Whether it can serve as a nonconformity score, calibrated on unlabelled multi-view recordings and then applied at single-view inference, is a well-posed question with a genuine obstacle: the exchangeability on which conformal validity rests is not obviously satisfied across viewpoints, and characterizing the conditions under which it holds would be the substance of the work rather than a preliminary to it.

\noindent \textbf{Can the body frame stop inheriting the estimator's failures?} SittingDown fails because the frame is built from two vectors and one of them, the hip axis, is short and poorly estimated in a seated pose. A frame estimated from all seventeen joints under a robust criterion, or regularized by a subject-specific skeleton accumulated online, should degrade more gracefully. The weakness is localized precisely enough by Section \ref{sec:h36mcv} to make this a concrete design problem rather than a general aspiration, and the multi-scale result already shows that frames built from limb-local evidence recover much of what the global frame loses on exactly that action.

These three share one objective: reliability estimates for frozen monocular predictors that remain valid under distribution shift. The contribution of the present project to that objective is largely negative, in that it eliminates a family of plausible signals and explains why they fail, and a study that took the problem further would sensibly begin from that elimination rather than repeat it.
