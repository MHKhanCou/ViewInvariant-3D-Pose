...
Monocular 3D human pose estimators produce skeletons expressed in the coordinate frame of the observing camera, so one motion recorded from two viewpoints yields two different sets of coordinates, preventing direct comparison in gait analysis and sports science. Existing remedies retrain the estimator, require calibration, or learn a canonicalization network, all assuming training data. We build a body-fixed frame from anatomical axes by Gram-Schmidt orthonormalization and apply it after prediction, adding no trained parameters to a frozen estimator, and we use it to ask what determines whether such a frame is consistent across viewpoints. We derive an answer: a direction read from two joints a distance $L$ apart, each carrying independent noise $\sigma$, is uncertain by approximately $2\sigma/L$, so a frame should be built from the longest available axis. We then test that principle at three levels, pre-registering each criterion before running it. It governs the choice between frame constructions, where rank correlation with cross-view consistency is 0.904 and 0.880 on two independently trained backbones, the implied $\sigma$ agrees to within one percent across them, and rebuilding each segment frame from its own long axis raises the improvement from 25.6 to 55.1 percent. It does not govern which frame within one construction to trust, nor which joint will disagree, because there articulation dominates: joints rigid with the torso sit at a larger radius than articulated ones yet disagree 2.5 times less. On Human3.6M, which played no part in developing the method, canonicalization reduces cross-view joint distance by 74.1 percent over all 180 held-out pairs, improving 179 and closing 90.5 percent of the gap to a Procrustes oracle requiring both views. The failures are reported in the same detail. An analytic reliability score is falsified as an accuracy predictor along five independent axes, though it does gate canonicalization quality on both backbones, and a bone-length signal reaching 0.492 on one dataset falls to 0.098 on the other, so we retract it. All five share one cause: geometric plausibility is invariant to coherent depth error.
}
...
\begin{itemize}
\item \textbf{The axis-length principle, derived from the construction rather than observed.} A direction read from two joints separated by $L$, each carrying independent noise $\sigma$, is uncertain by approximately $2\sigma/L$, so a frame should be built from the longest axis available. On two independently trained backbones of different architecture and nineteen-fold different size, rank correlation with cross-view consistency is 0.904 and 0.880, and the implied physical constant agrees to within one percent. Acting on it raises the multi-scale improvement from 25.6 to 55.1 percent at no parameter cost.
...
One finding is genuinely a design result rather than an evaluation, and we regard it as the most transferable thing in this report, since any method that builds a frame from anatomical landmarks faces the same choice. The consistency of a body frame across viewpoints is governed by the length of the axis it is built from. We predicted it from the construction, discovered it through a bilateral asymmetry we had not gone looking for, confirmed it by correcting that asymmetry, generalized it to a second case the same reasoning identified, and then derived it from first principles as $\theta \approx 2\sigma/L$. It raises the multi-scale improvement from 25.6 to 55.1 percent at no parameter cost, and the physical constant it implies agrees to within one percent across two independently trained networks of different architecture and nineteen-fold different size, which is not what an accidental correlation produces.
