...
\item \textbf{[Systems]} \textbf{A framework and an evidence trail built so the claims can be checked rather than believed.} Zero learned parameters added to a frozen estimator, 402 floating-point operations per frame, no labels and no camera parameters at any stage; 292 numerical claims checked against stored artifacts by an automated audit; 76 tests; and nine pre-registrations committed to version history before the experiments they govern, five of which failed their own criteria and one of which returned a competing method as the better one (Section \ref{sec:template}).
\end{itemize}
